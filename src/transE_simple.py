...
class TransE:
    ...
    def update_embeddings(self, Tbatch):
        # 这里深拷贝了整个字典,会造成训练速度较慢，但是是易于理解的，增加训练速度的版本是transE_2.0
        ...
        for triple, corrupted_triple in Tbatch:
            ...
            if err > 0: # 这里没懂？
                ...
                if triple[0] == corrupted_triple[0]:  # 若替换的是尾实体，则头实体更新两次
                    h_correct_update -= (-1) * self.learning_rate * grad_neg # 正例更新一次
                    t_corrupt_update -= self.learning_rate * grad_neg # 负例更新一次

                elif triple[1] == corrupted_triple[1]:  # 若替换的是头实体，则尾实体更新两次
                    h_corrupt_update -= (-1) * self.learning_rate * grad_neg  # 正例更新一次
                    t_correct_update -= self.learning_rate * grad_neg  # 负例更新一次
